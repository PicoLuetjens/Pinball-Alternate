//Paddle Object class which contains the paddleShape and the physical Paddle Object
class Paddle
{
  //Attributes
  Body body;
  ...
  //Constructors
  Paddle(float pX, float pY, float pSpeed, float pLowerAngle, float pUpperAngle, boolean pLeft, float r1, float r2, float flen)
  {
    ...
    if(width == 1920)
      rjd.maxMotorTorque = 1500000.0;
      //rjd.maxMotorTorque = 800000.0;
    else
  ...
  void makeBody(Vec2 center, boolean left)
  {
    ...
    if(left)
    {
      body.setTransform(pos, angle + HALF_PI+22*PI/180.);
